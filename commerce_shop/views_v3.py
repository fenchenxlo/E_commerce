...
def purchased_products(request):
    ...
    for order in orders:
        ...
        # 🔑 打包必要欄位 dict
        payment_payload = {
            "MerchantTradeNo": f"ORDER{order.id}",   # 商店交易編號
            "TradeAmt": float(total_with_shipping),  # 金額
            "PaymentType": "Credit",                 # 模擬付款方式
            "ReturnURL": "http://127.0.0.1:8001/ecpay/notify/",  # 模擬 notify URL
            "OrderID": order.id,
