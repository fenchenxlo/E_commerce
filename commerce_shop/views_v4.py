...
import uuid, datetime, hashlib, time
...
#         order_total = 0  # 先建立訂單小計（未加運費）,移到迴圈外,避免每次重設
        ...
        payment_payload = {
            ...
            "MerchantTradeDate": datetime.datetime.now().strftime("%Y/%m/%d %H:%M:%S"),  # 自動產生交易時間
            "UserId": user_id,
            "TradeAmt": float(total_with_shipping),
            ...
            "OrderResultURL": "http://127.0.0.1:8000/commerce_shop/payment-ok/",  # browser redirect ,瀏覽器跳轉 URL
#             "BankCode": bank_code,
...
def payment_ok(request):
    ...
    for order in orders:
        ...
        order_list.append({
            ...
            "payments": order.payments.all()  # 付款紀錄
        })
...
#         "MerchantID": settings.ECPAY_MERCHANT_ID,
    ...
    <body>        
        ...
        <form id='ecpay-form'
              ...
              action='http://127.0.0.1:8001/ecpay_mock/ecpay_mock_pay'
