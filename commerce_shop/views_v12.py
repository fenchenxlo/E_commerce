...
@login_required
def search_product_list(request,search_key):
    
    if search_key in name or search_key in regex_name:
        
        # 顯示登入會員自己擁有的商品，並過濾掉下架商品
        products = list(Product.objects.filter(name=search_key , is_deleted=False).order_by('-created_at'))
...
#                 "paid_at": datetime.datetime.strptime(paid_at, "%Y-%m-%d %H:%M:%S"),
        ...
        # 發現沒有 Payment 紀錄時，自動補建一筆，避免程式炸掉
...
#                 "paid_at": datetime.datetime.strptime(paid_at, "%Y-%m-%d %H:%M:%S"),
            ...
            bank_code=data.get("bank_code"),
            v_account=data.get("v_account"),
            expire_date=datetime.datetime.strptime(data.get("expire_date"), "%Y-%m-%d %H:%M:%S"),         
        )
