...
#         }
    ...
    try:
        if request.method == "POST":
            ...
            merchant_trade_no = request.POST.get("merchant_trade_no")  # 商城訂單編號
#             print("merchant_trade_no: ",merchant_trade_no)
            merchant_trade_date = request.POST.get("merchant_trade_date")
            user_id = request.POST.get("user_id")
            trade_amt = int(request.POST.get("trade_amt"))
            trade_desc = request.POST.get("trade_desc")
            item_name = request.POST.get("item_name")
            choose_payment = request.POST.get("choose_payment") or "ATM"
            payment_type = request.POST.get("payment_type") or "aio"
            transaction_id = request.POST.get("trade_no")
            status = request.POST.get("status")
            # 🔑 綠界官方定義
            ReturnURL = request.POST.get("return_url")
            PaymentInfoUrl = request.POST.get("payment_info_url")
            OrderResultURL = request.POST.get("order_result_url")
            bank_code = request.POST.get("bank_code")
            v_account = request.POST.get("v_account")
    ...
    for order in orders:
        ...
        order_list.append({
            ...
            "total_with_shipping": order.total_amount + order.shipping_fee,
            "payment_payload": context  # 從綠界回來的 payment_load
        })
